# Setting Up
When you clone the project, there are five steps that must be completed to get the project running.
...
4. In the `backend/` folder, run `npm start` to start the backend.
5. In the `frontend/` folder, run `npm start` to start the frontend.
